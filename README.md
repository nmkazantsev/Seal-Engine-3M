...
Ниже приведены дополнения к документации, которые необходимо вставить в файл `docs.md`:

1. **Дополнение в раздел 7 «Освещение и материалы»** – вставьте после описания класса `Material` (перед разделом 8 «Текстуры»).
2. **Новый раздел 15 «Отладка»** – вставьте после раздела 14 «Дополнительные классы» (перед «Заключением»).
...
2. Загрузить и скомпилировать шедйеры, передать в них матрицу преобразования
2. В `onSurfaceChanged` инициализировать камеру (`Camera`), загрузить ресурсы. Передать камеру в шейдер.
