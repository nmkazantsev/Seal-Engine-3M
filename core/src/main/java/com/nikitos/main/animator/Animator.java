package com.nikitos.main.animator;


import com.nikitos.CoreRenderer;
import com.nikitos.main.engine_object.SealObject;

import java.util.HashMap;
import java.util.function.Function;

import static com.nikitos.utils.Utils.contactArray;
import static com.nikitos.utils.Utils.popFromArray;
...
public class Animator {
    ...
    public static void unfreezeAnimations(SealObject target) {
        ...
        for (Animation a: animQueue.get(target)) {
            if (!a.waiting) continue;
            a.waiting = false;
            a.elapsedTime = CoreRenderer.engine.pageMillis() - a.freezeTiming;
        }
    }
    ...
    public static class Animation {
        ...
        private Animation(SealObject target, Function<Animation, float[]> tf, float[] args, Function<float[], Float> vf, float duration, float vfa, long st, boolean recurring) {
            long c = CoreRenderer.engine.pageMillis();
            if (st <= c) {
                startTiming = c;
                isActive = true;
            } else {
        ...
        public float[] getAnimMatrix() {
            if (waiting) {
                if (!frozen) {
                    frozen = true;
                    freezeTiming = CoreRenderer.engine.pageMillis();
                }
                return attrs;
            }
            if (!isActive) {
                if (startTiming + elapsedTime <= CoreRenderer.engine.pageMillis()) {
                    isActive = true;
                    return getAnimMatrix();
                }
                return attrs;
            }
            float gt = (CoreRenderer.engine.pageMillis() - (startTiming + elapsedTime)) / duration; // global timing (linear from 0 to 1)
            ...
            if (gt >= 1) {
                if (recurring) {
                    startTiming = CoreRenderer.engine.pageMillis();
