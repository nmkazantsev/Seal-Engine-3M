...
public class CoreRenderer {
    ...
    public CoreRenderer(float width, float height, Engine engine) {
        ...
        frameCaptureSource = runtimeObserver == null ? null : pf.getFrameCaptureSource();
        float x = Utils.getX();
        float y = Utils.getY();
        float kx;
        float ky;
    ...
    public void draw() {
        ...
        // Здесь сцена и отложенный ввод уже обработаны, а swap buffers ещё не произошёл.
        if (runtimeObserver != null) {
            runtimeObserver.afterFrame(frameCaptureSource);
        }
    }
}
